...
async def main():
    ...
    with sshtunnel.SSHTunnelForwarder(
        ...
        with (
            ...
            cur.execute(
                "SELECT id, user_id, problemID, score, code FROM submissions WHERE id = %s",
                (17,),
            )
            row = cur.fetchone()
            if row:
                print(
                    f"id={row[0]} user_id={row[1]} problem_id={row[2]} score={row[3]}"
                )
                print("Code:")
                print(row[-1])
            else:
                print("No submission found with the specified ID.")
